...
class SkipCNN(CNN):
    ...
    def building(self, layer, graph, skip):
        ...
        with graph.as_default():
            for ind in range(self.descriptor.number_hidden_layers):
                if skip == ind:
                    layer = tf.pad(tensor=layer, paddings=tf.constant([[0, 0], [0, self.List_layers[0].shape[1]-layer.shape[1]], [0, self.List_layers[0].shape[2]-layer.shape[2]], [0, 0]]))
                    layer = tf.concat((layer, self.List_layers[0]), axis=3)
                if self.descriptor.layers[ind] == 2:  # If the layer is convolutional
                    layer = tf.nn.conv2d(input=layer, filters=self.List_weights[ind], strides=(1, self.descriptor.strides[ind][0], self.descriptor.strides[ind][1], self.descriptor.strides[ind][2]), padding=[[0, 0], [0, 0], [0, 0], [0, 0]])
                elif self.descriptor.layers[ind] == 0:  # If the layer is average pooling
                    layer = tf.nn.avg_pool2d(input=layer, ksize=(1, self.descriptor.filters[ind][0], self.descriptor.filters[ind][1], 1), strides=(1, self.descriptor.strides[ind][0], self.descriptor.strides[ind][1], 1), padding="VALID")
                else:
                    layer = tf.nn.max_pool2d(input=layer, ksize=(1, self.descriptor.filters[ind][0], self.descriptor.filters[ind][1], 1), strides=(1, self.descriptor.strides[ind][0], self.descriptor.strides[ind][1], 1), padding="VALID")
...
def train_cnn(nets, placeholders, sess, graph, train_inputs, train_outputs, batch_size, hypers):
    ...
    with graph.as_default():
        ...
        out = tf.compat.v1.layers.dense(tf.compat.v1.layers.flatten(out), 20)
        predictions["n0"] = out
        out = nets["n1"].building(predictions["n0"], graph, '_')
        predictions["n1"] = out
...
if __name__ == "__main__":
    ...
    e = evolution.Evolving(loss=train_cnn, desc_list=[ConvDescriptor, MLPDescriptor], x_trains=[x_train], y_trains=[y_train], x_tests=[x_test], y_tests=[y_test],
                 evaluation=eval_cnn, batch_size=150, population=200, generations=10, n_inputs=[[28, 28, 3], [20]], n_outputs=[[20], [10]], cxp=0.5,
